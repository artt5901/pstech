<?php
	...
	if(!empty($s_username)&&!empty($s_name)&&!empty($$s_idcard)
	/*&&!empty($f_id)&&!empty($f_name)&&!empty($f_tel)&&!empty($m_id)&&!empty($m_name)&&!empty($m_tel)&&!empty($pa_id)&&!empty($pa_name)&&!empty($pa_tel)&&!empty($pa_relation)*/){
		...
		$sql3 = "SELECT * FROM student WHERE s_idcard = '$s_idcard' ";
		...
		if($total == 0){
			/*if($images != "")
				copy($images,"./image/".$images_name);*/
					$sql = "INSERT INTO student (s_username, s_password, s_name, s_idcard, s_address, s_tel, s_email, s_year, s_hbd, b_id, st_id, level_id, s_brethren, s_child, f_id, m_id, pa_id) VALUES ('$s_username', '$s_password', '$s_name', '$s_idcard', '$s_address', '$s_tel', '$s_email', '$s_year', '$s_hbd', '$b_id', '$st_id', '$level_id', '$s_brethren', '$s_child', '$f_id', '$m_id', '$pa_id' )";
		mysqli_query($conn,$sql);
					
					$sql2 = "INSERT INTO father (f_id ,f_name ,f_tel) VALUES ('$f_id','$f_name','$f_tel')";			
		mysqli_query($conn,$sql2);
		
					$sql3 = "INSERT INTO mathar (m_id ,m_name ,m_tel) VALUES ('$m_id','$m_name','$m_tel')";
		mysqli_query($conn,$sql3);
		
					$sql4 = "INSERT INTO parent (pa_id, pa_name, pa_address, pa_relation, pa_metier, pa_tel) VALUES ('$pa_id', '$pa_name', '$pa_address', '$pa_relation', '$pa_metier', '$pa_tel')";
		mysqli_query($conn,$sql4)
		or die("8. ไม่สามารถประมวลผลคำสั่งได้");
		mysqli_close($conn);
